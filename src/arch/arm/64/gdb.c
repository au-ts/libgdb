...
uint32_t read_word(uint16_t client, uintptr_t addr, seL4_Word *val) {
    ...
    *val = *ptr_to_page;
    microkit_dbg_puts("We did a successful read word!\n");
    return 0;
}
...
uint32_t write_word(uint16_t client, uintptr_t addr, seL4_Word val) {
    ...
    uint64_t *ptr_to_page = (uint64_t *) (0x600000 + (addr & 0xfff));
    *ptr_to_page = val;
    microkit_dbg_puts("We did a successful write word!\n");
    return 0;
}
...
bool set_software_breakpoint(gdb_inferior_t *inferior, seL4_Word address) {
    ...
    // if (seL4_ARM_VSpace_Write_Word(inferior->vspace, address, ret)) {
    //     return false;
    // }
...
bool unset_software_breakpoint(gdb_inferior_t *inferior, seL4_Word address) {
    for (int i = 0; i < MAX_SW_BREAKS; i++) {
        if (inferior->software_breakpoints[i].addr == address) {
            ...
            if (err) {
                microkit_dbg_puts("We got an error when trying to write to address - 3\n");
                return false;
            } else if (!err) {
...
char *inf_mem2hex(gdb_thread_t *thread, seL4_Word mem, char *buf, int size, seL4_Word *error)
{
    ...
    for (i = 0; i < size; i++) {
        if (i % sizeof(seL4_Word) == 0) {
            ...
            if (err) {
                // microkit_dbg_puts("We got an error when trying to read address - 4\n");
                *error = err;
                return NULL;
            }
...
seL4_Word inf_hex2mem(gdb_thread_t *thread, char *buf, seL4_Word mem, int size)
{
    ...
    for (i = 0; i < size; i++, mem++) {
        if (i % sizeof(seL4_Word) == 0) {
            ...
            uint32_t err = read_word(thread->inferior->gdb_id, mem, &ret);
            if (err) {
                // microkit_dbg_puts("We got an error when trying to read address - 5\n");
                return (mem + i);
            }
